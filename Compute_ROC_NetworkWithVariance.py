...
def NetworkWithVar(X_train, Y_train, Model):
    ...
    W_out_one = graph.get_tensor_by_name("W_out_one:0")
    W_out_two = graph.get_tensor_by_name("W_out_two:0")
    ...
    prediction, variance = multilayer_perceptron(tf.cast(X_train, tf.float32), W_hid, W_out_one, W_out_two)
...
sess = tf.Session()
pred_G01, Res_G01 = NetworkWithVar(X_G01, Y_G01, 'NetworkWithVariance-40')
pred_G02, Res_G02 = NetworkWithVar(X_G02, Y_G02, 'NetworkWithVariance-41')
pred_G03, Res_G03 = NetworkWithVar(X_G03, Y_G03, 'NetworkWithVariance-42')
pred_G04, Res_G04 = NetworkWithVar(X_G04, Y_G04, 'NetworkWithVariance-43')
...
np.save("Results_nofault_Network.npy", Results_nofault)

#########################################################################################


Results_nofault = np.load("Results_nofault_Network.npy")

TimePeriod = [2, 4, 6, 12, 25, 40, 60, 90, 120]
count = 0
    ...
    for f in files:
        if not f.startswith('G'):
            ...
            try:
                ...
                sess = tf.Session()
                pred_fault, Res = NetworkWithVar(X_failure, Y_failure, 'NetworkWithVariance-' + str(file + 1))
...
plt.gca().set_title('ANN(' + r'$\mu$' + ',' + r'$\sigma$' + ')')
